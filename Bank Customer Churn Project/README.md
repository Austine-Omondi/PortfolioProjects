...
An end-to-end data analytics project built with **MySQL** and **Tableau Public**. The goal was to identify customer groups with elevated churn risk and support more focused retention analysis.

## Project objective

The project addresses a practical stakeholder question:
...
The analysis covers **10,000 customer records** and compares churn by country, age, number of products, activity status, gender, and combined customer segments.
...
- **2,037 of 10,000 customers churned**, resulting in an overall churn rate of **20.37%**.
...
- Customers with **three or four products** had very high churn rates, but the groups contained only **266** and **60** customers respectively.
...
- Initially stored fields with uncertain data types as text.
- Inspected identifiers, categories, binary fields, numeric formats, and ranges before converting the data types.
- Converted validated fields to appropriate data types.
- Checked for missing values, duplicate customer IDs, whitespace, invalid categories, and illogical ranges.
...
- Displayed churn rate alongside churned-customer count to account for both risk concentration and business impact.
...
- The extreme churn rates among customers with three or four products should be interpreted cautiously because the groups are small.
...
- Review the experience of customers with three or four products while accounting for the small group sizes.
- Use churn rate and churned-customer count together when prioritising segments.

## Limitations

The dataset is a customer snapshot. It does not show behaviour over time or explain why customers left. It also does not include transaction history, service interactions, customer feedback, or campaign outcomes. The findings should guide further investigation and controlled retention tests, not be treated as causal conclusions.
